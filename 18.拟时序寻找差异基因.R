...
dir.create("Monocle3改")
#//创建CDS对象并预处理数据
stromal <- readRDS("stromal.rds")
table(stromal@meta.data$celltype)
CAF <- subset(CAF,celltype=='demosplastic CAFs'|celltype=="inflammatory CAFs"|celltype=="myofibroblast CAFs")
data <- GetAssayData(CAF, assay ='RNA', slot = 'counts')
cell_metadata <- CAF@meta.data
...
colData(cds)
plot_cells(cds,color_cells_by="celltype",cell_size = 1,graph_label_size = 5,group_label_size= 4)
cds <- cluster_cells(cds) 
#轨迹学习Learn the trajectory graph（使用learn_graph()函数）
## 识别轨迹
cds <- learn_graph(cds)
plot_cells(cds, color_cells_by = "celltype", label_groups_by_cluster=FALSE,label_leaves=FALSE, label_branch_points=FALSE,group_label_size= 4)
...
genes <- c('SFRP2','POSTN','PRRX1')
genes <- c('SFRP2','POSTN','PRRX1',"CTSK","SULF1")
plot_genes_in_pseudotime(cds[genes,],color_cells_by="celltype",min_expr=0.5, ncol= 2)
#FeaturePlot图
...
pseudotime <-pseudotime[rownames(CAF@meta.data)]
CAF$pseudotime <- pseudotime
p = FeaturePlot(CAF, reduction ="umap", features = "pseudotime")
p
ggsave("Pseudotime_Seurat.pdf",plot = p, width = 8, height = 6)
saveRDS(CAF, file ="sco_pseudotime.rds")
