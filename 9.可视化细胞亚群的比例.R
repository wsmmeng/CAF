...
unique(OV$sample)
table(OV$celltype)
index <- which(OV$group=='OVH')
OV@meta.data$group[index] <- 'GSE158937'
index <- which(OV$group=='OVPPP')
OV@meta.data$group[index] <- 'GSE165897'
index <- which(OV$group=='OVI')
OV@meta.data$group[index] <- 'GSE165897'
index <- which(OV$group=='OVP')
OV@meta.data$group[index] <- 'GSE147892'
table(OV@meta.data$group)
##饼图
library(plotrix)
library(dplyr)
library(ggsci)
mynames <-   table(OV$celltype) %>% names()
myratio <-  table(OV$celltype) %>% as.numeric()
...
# 绘图
df <- table(OV$celltype) %>% as.data.frame()
...
cellnum <- table(OV$celltype,OV$dataset)
...
cellnum <- table(OV$sample,OV$celltype)
cellnum[1:4,1:4]
##             
...
cellnum$Group <- factor(cellnum$Group,levels = c('GSE165897','GSE147082','GSE158937'))
...
compaired <- list(c('GSE165897','GSE147082'),
                  c('GSE165897',"GSE158937"),
                  c("GSE147892","GSE158937"))
...
library(ggplot2)
## Warning: package 'ggplot2' was built under R version 4.1.3
cellnum <- table(OV$celltype,OV$sample)
cell.prop<-as.data.frame(prop.table(cellnum))
colnames(cell.prop)<-c("Celltype","Sample","Proportion")

  
p.bar <- ggplot(cell.prop,aes(Sample,Proportion,fill=Celltype))+
  geom_bar(stat="identity",position="fill")+
  ggtitle("cell proportion")+
  theme_bw()+
  coord_flip()
