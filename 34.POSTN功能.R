...
cell<- names(sort(CAF@assays$RNA@counts['POSTN',],
                  decreasing =T))
POSTNexpr <- c(rep('high',3668),rep('low',3668)) 
newmeta<-data.frame(cell,POSTNexpr)
...
##加回去
CAF <- AddMetaData(CAF,metadata = newmeta[,2],col.name = 'POSTNexpr')
Idents(CAF) <- 'POSTNexpr'
p1 <- VlnPlot(CAF,features = 'POSTN')
p1

#比较B_cell和T_cells的差异表达基因
Idents(CAF) <- 'POSTNexpr'
dge.cellPOSTNp <- FindMarkers(CAF, ident.1 = 'high', ident.2 = 'low',logfc.threshold = 0.05,group.by = 'POSTNexpr')
save(dge.cellPOSTNp,file='dge.cellPOSTNp.Rdata')


##
markers <- rownames(dge.cellPOSTNp)
...
head(kk)[,1:6]
KKPOSTNresult <- kk@result
pathway <- KKPOSTNresult[c("hsa00190","hsa04810","hsa04141",	"hsa03018","hsa04510"),]
library(DOSE)
...
save(ego_BPresultPOSTN,file = 'ego_BHresultPOSTN.Rdata')
