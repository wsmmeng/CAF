...
cell<- names(sort(CAF@assays$RNA@counts['PRRX1',],
                  decreasing =T))
PRRX1expr <- c(rep('high',3668),rep('low',3668)) 
newmeta<-data.frame(cell,PRRX1expr)
...
##加回去
CAF <- AddMetaData(CAF,metadata = newmeta[,2],col.name = 'PRRX1expr')
Idents(CAF) <- 'PRRX1expr'
p1 <- VlnPlot(CAF,features = 'PRRX1')
p1


#比较B_cell和T_cells的差异表达基因
Idents(CAF) <- 'PRRX1expr'
dge.cellPRRX1p <- FindMarkers(CAF, ident.1 = 'high', ident.2 = 'low',logfc.threshold = 0.05,group.by = 'PRRX1expr')
save(dge.cellPRRX1p,file='dge.cellPRRX1p.Rdata')
...
##
markers <- rownames(dge.cellPRRX1p)
...
head(kk)[,1:6]
KKPRRX1result <- kk@result
pathway <- KKPRRX1result[c("hsa04218","hsa05208","hsa04144","hsa05205"),]
library(DOSE)
