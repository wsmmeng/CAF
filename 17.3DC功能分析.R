...
DCmarker <- subset(celltypemarkers,cluster=='demosplastic CAFs')
DCgene <- DCmarker$gene
gene <- DCmarker$gene
...
head(kk)[,1:6]
KKDCresult <- kk@result
pathway <- KKDCresult[c("hsa04974","hsa04510","hsa04512"),]
library(DOSE)
...
ggplot(pathway,aes(GeneRatio,Description))+
  ...
  labs(color=expression(pvalue),size="Count", ##expression函数定义函数样式 []添加下标，^添加上标
       x="GeneRatio", ##自定义标轴
       y="",
       title="KEGG Pathway enrDChment")+ ##自定义坐标轴
  theme_bw() #去掉背景

save(ego_BPresultDC,file = 'ego_BHresultDC.Rdata')
