...
markers <- FindAllMarkers(stromal,
                          only.pos = TRUE,
                          min.pct = 0,
                          logfc.threshold = 0)
save(markers,file='celltypemarkers.Rdata')
...
col <- pal_npg()(7)
DoHeatmap(stromal,features = top10markers$gene,
...
DimPlot(stromal, label = F,   
       ...
       NoLegend() 
#默认作图  
FeaturePlot(stromal, features = c("POSTN","SFRP2","PRRX1","CTSK","SULF1"))  
#修饰图片  
mycolor <- c('gray', 'white','red')#设置颜色  
FeaturePlot(stromal, features = c("POSTN"),cols = mycolor, pt.size = 1,max.cutoff = 2.5)+  
  theme(panel.border = element_rect(fill=NA,color="black", size=1, linetype="solid"))#加边框
VlnPlot(stromal, features = c("POSTN"))
FeaturePlot(stromal, features = c("SFRP2"),cols = mycolor, pt.size = 1,max.cutoff = 2.5)+  
  theme(panel.border = element_rect(fill=NA,color="black", size=1, linetype="solid"))#加边框
Idents(stromal) <- 'celltype'
VlnPlot(stromal, features = c("SFRP2"))
FeaturePlot(stromal, features = c("PRRX1"),cols = mycolor, pt.size = 1,max.cutoff = 1.8)+  
  theme(panel.border = element_rect(fill=NA,color="black", size=1, linetype="solid"))#加边框
VlnPlot(stromal, features = c("PRRX1"))
FeaturePlot(stromal, features = c("CTSK"),cols = mycolor, pt.size = 1,max.cutoff = 1.8)+  
  theme(panel.border = element_rect(fill=NA,color="black", size=1, linetype="solid"))#加边框
FeaturePlot(stromal, features = c("SULF1"),cols = mycolor, pt.size = 1,max.cutoff = 1.8)+  
  theme(panel.border = element_rect(fill=NA,color="black", size=1, linetype="solid"))#加边框
...
Idents(stromal) <- 'celltype'
DotPlot(stromal, features = markers)+coord_flip()+theme_bw()+#去除背景，旋转图片  
...
##画个气泡图
markers <- c('CTHRC1',"FN1","COL11A2","COL1A1",##dCAF
             "RSAD2",'PDPN',##lymphatic endothelial cell
...
DotPlot(stromal,features = markers)+coord_flip()+
